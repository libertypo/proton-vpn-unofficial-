...
mod tests {
    ...
    async fn test_stdin_key_with_standard_output() {
        let fake_private_key = "TEST_PRIVATE_KEY_PLACEHOLDER";
        let output = run_dialog(
            default_args(),
            "SECRET_KEY=private-key\n\
             SECRET_VAL=TEST_PRIVATE_KEY_PLACEHOLDER\n\
             DONE\n",
            MockKeyring { secret: None },
        )
        .await;

        assert_eq!(output, format!("private-key\n{}\n\n\n", fake_private_key));
    }
    ...
    async fn test_keyring_with_standard_output_output() {
        ...
        let output = run_dialog(
            default_args(),
            "DONE\n",
            MockKeyring { secret: Some(fake_private_key.to_string()) },
        )
        .await;
