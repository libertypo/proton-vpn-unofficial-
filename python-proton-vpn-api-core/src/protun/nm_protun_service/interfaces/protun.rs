...
fn connection_owner(user: Option<u32>) -> zbus::fdo::Result<u32> {
    user.ok_or(zbus::fdo::Error::AccessDenied(
        "no connection owner set".into(),
    ))
}
...
async fn check_caller_is_connection_owner(
    ...
    let owner = connection_owner(state.read().await.user.clone())?;
...
mod tests {
    ...
    #[test]
    fn denies_missing_connection_owner() {
        assert!(connection_owner(None).is_err());
    }

    #[test]
    fn returns_connection_owner() {
        assert_eq!(connection_owner(Some(1000)).unwrap(), 1000);
    }
}
