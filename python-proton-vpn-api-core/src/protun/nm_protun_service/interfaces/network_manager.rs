        ...
        let connection_info = service
            .connect(
                InitialConnectionConfig {
                    ...
                    pcap_file: None,
                    connection_mode: protun::api::connection::ConnectionMode::NoLocalAgent {
...
impl NetworkManager {
    ...
    async fn connect(
        ...
        Self::emit_nm_config(&emitter, config_info).await?;
    ...
    async fn connect_interactive(
        ...
        let caller_uid = Self::caller_uid(connection, &header).await?;
        log::info!("ConnectInteractive caller uid: {caller_uid}");
        let params = load_connection_params_from_settings_with_user(settings, Some(caller_uid))?;
        log::info!("Using interface name: {}", params.ipv4_interface.name);
        self.set_state(&emitter, NMVpnServiceState::Starting).await?;
        let config_info = self.establish_connection(params).await?;
        log::info!("TUN interface {} created", config_info.ipv4_interface.name);
