...
impl protun::api::connection::PersistentCache for NoopCache {
    fn put(&self, _key: protun::api::connection::CacheKey, _bytes: Vec<u8>) {}
    fn get(&self, _key: protun::api::connection::CacheKey) -> Option<Vec<u8>> { None }
    fn clear(&self) {}
}
...
impl Service {
    ...
    pub fn update_wg_private_key(
        ...
        let _ = private_key;
        log::warn!("WireGuard private-key rotation is not supported by the current protun Connection API; the key is set at connect time.");
        Ok(())
    }
