...
class LinuxNetworkManager(VPNConnection):
    ...
    async def start(self):
        ...
        try:
            ...
            self._notify_subscribers(
                events.TunnelSetupFailed(
                    context=EventContext(connection=self, error=NM.VpnConnectionStateReason.NONE.real)
                )
            )
            return
        ...
        try:
            ...
            self._notify_subscribers(events.TunnelSetupFailed(context=EventContext(connection=self, error=error)))
            await self.remove_connection(connection)
    ...
    def validate(cls):
        ...
        if not nm_client.is_nm_version_compatible():
            logger.error("%s: NetworkManager version is not compatible, VPN connections may fail.", cls.__name__)
            return False
        return True
