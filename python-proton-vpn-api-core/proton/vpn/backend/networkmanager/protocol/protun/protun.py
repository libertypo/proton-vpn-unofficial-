...
class Protun(LinuxNetworkManager, LocalAgentMixin):
    ...
    @staticmethod
    def _native_nm_artifacts_present() -> bool:
        """Returns whether the required NetworkManager runtime artifacts are installed."""
        required_paths = (
    ...
    def validate(cls):
        ...
        if not cls.plugin_exists:
            return False
        if not cls._native_nm_artifacts_present():
            logger.warning(
                "%s: required native NetworkManager ProTun artifacts are missing; Protocol is unavailable.",
                cls.__name__,
            )
            return False
        return True
