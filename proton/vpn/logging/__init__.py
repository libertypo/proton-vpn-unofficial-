...
def _anonymize_ip(value: str) -> str:
    """Mask device-origin country/IP fields while keeping server details visible."""

    def _mask_ip_value(candidate: str) -> str:
        try:
            ipv4 = ipaddress.IPv4Address(candidate)
        ...
        try:
            ipv6 = ipaddress.IPv6Address(candidate)
            exploded = ipv6.exploded.split(":")
            return f"{exploded[0]}:{exploded[1]}:{exploded[2]}:xxxx:xxxx:xxxx:xxxx:xxxx"
        except ipaddress.AddressValueError:
            return candidate

    def _mask_device_ip(match: re.Match) -> str:
        prefix, candidate, suffix = match.groups()
        return f"{prefix}{_mask_ip_value(candidate)}{suffix}"

    anonymized = _DEVICE_COUNTRY_PATTERN.sub(r"\1XX\3", value)
    anonymized = _DEVICE_IP_PATTERN.sub(_mask_device_ip, anonymized)
    return anonymized


class _IPAnonymizingFilter(logging.Filter):
    """Filter that anonymizes IP addresses before the message is formatted."""

    def filter(self, record: logging.LogRecord) -> bool:
        message = record.getMessage()
        masked_message = _anonymize_ip(message)
...
def config(filename, logdirpath=None, log_to_console=True):
    ...
    if anonymize_ips:
        _handler_file.addFilter(_IPAnonymizingFilter())
    ...
    if log_to_console:
        ...
        if anonymize_ips:
            _handler_console.addFilter(_IPAnonymizingFilter())
