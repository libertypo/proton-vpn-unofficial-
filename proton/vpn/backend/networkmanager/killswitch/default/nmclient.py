        ...
        def _on_connection_added(nm_client, res, _user_data):
            try:
                ...
                future_conn_activated.set_exception(
                    RuntimeError(f"Error setting adding KS connection: {nm_client=}, {res=}").with_traceback(
                        exc.__traceback__
                    )
                )
                return
        ...
        def _on_connection_removed(connection, result, _user_data):
            try:
                ...
                future_interface_removed.set_exception(
                    RuntimeError(f"Error removing KS connection: {connection=}, {result=}").with_traceback(
                        exc.__traceback__
                    )
                )
