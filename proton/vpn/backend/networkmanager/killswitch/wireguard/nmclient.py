        ...
        def _on_connection_activated(nm_client, res, _user_data):
            try:
                ...
                future_conn_activated.set_exception(
                    RuntimeError(f"Error activating KS connection: {exc}").with_traceback(exc.__traceback__)
                )

        def _on_connection_added(nm_client, res, _user_data):
            try:
                added_connection = nm_client.add_connection_finish(res)
                nm_client.activate_connection_async(
                    added_connection,
                    None,
                    None,
                    None,
                    _on_connection_activated,
                    None,
                )
            except Exception as exc:  # pylint: disable=broad-except
                future_conn_activated.set_exception(
