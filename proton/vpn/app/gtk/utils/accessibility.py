...
gtk_version = Version(f"{Gtk.get_major_version()}.{Gtk.get_minor_version()}.{Gtk.get_micro_version()}")
accessible_list_available = gtk_version >= Version("4.14.0")
if not accessible_list_available:
    logger.warning("Upgrade for better UI accessibility. Gtk.AccessibleList requires GTK >= 4.14.0")
...
def remove_accessibility(target_widget: Gtk.Widget, relation_type: Gtk.AccessibleRelation):
