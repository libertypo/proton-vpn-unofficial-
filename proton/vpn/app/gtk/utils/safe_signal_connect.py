...
def safe_signal_connect(
    emitter: GObject.Object, signal: str, callback: Callable[..., Any], *, once: bool = False
) -> int:
    ...
    if not callable(callback):
        raise TypeError(f"safe_signal_connect callback must be callable, got {type(callback).__name__}.")
...
def _connect_weak(
    ...
    *emitter* itself is destroyed.
    ...
    receiver_repr = f"{type(method.__self__).__name__}.{method.__func__.__name__}"
