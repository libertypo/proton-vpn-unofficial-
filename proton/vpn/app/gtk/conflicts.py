...
SPLIT_TUNNEL_PROTOCOL_CONFLICT = "•  This will automatically set your protocol to WireGuard."
...
def _supports_split_tunneling(protocol: str) -> bool:
    return protocol.startswith(PROTUN_PROTOCOL_PREFIX) or protocol == WIREGUARD_PROTOCOL
...
class Conflicts:
    ...
    @staticmethod
    def detect(setting_type: str, setting: str, value: Any, settings: Union[Settings, AppConfig]) -> Optional[Conflict]:
        """
        ...
        if msg:
            return Conflict(label=label, description="\n".join(msg))
        return None

    @staticmethod
    def resolve(
        setting_type: str, setting: str, value: Any, settings: Union[Settings, AppConfig]
    ) -> Union[Settings, AppConfig]:
        """
        ...
        if setting_type == "settings":
            ...
            if setting == "features.split_tunneling.enabled" and bool(value):
                if not _supports_split_tunneling(runtime_settings.protocol):
                    runtime_settings.protocol = WIREGUARD_PROTOCOL
                runtime_settings.killswitch = 0

            # Kill switch conflicts
            if setting == "killswitch" and value != 0:
                runtime_settings.features.split_tunneling.enabled = False
