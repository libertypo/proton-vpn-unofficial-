...
SPLIT_TUNNELING_APP_RESTART_MESSAGE = "Split tunneling enabled. Remember to restart affected apps."


class VPNConnectionStatusWidget(Gtk.Box):  # pylint: disable=too-many-instance-attributes
    """Displays the current connection status."""

    MAXIMUM_SESSIONS_ERROR = (
        "You've reached your maximum device limit. To reconnect to VPN, please disconnect from another device."
    )
    ...
    def __init__(
        ...
        self._protected_icon: Gio.Icon
        self._unprotected_icon: Gio.Icon
        self._fastest_icon: Gio.Icon
    ...
    def _build_status_box(self) -> Gtk.Box:
        ...
        self._protected_icon = Gio.FileIcon.new(
            Gio.File.new_for_path(str(icons.ICONS_PATH / Path("connection-status/protected.svg")))
        )
        self._unprotected_icon = Gio.FileIcon.new(
            Gio.File.new_for_path(str(icons.ICONS_PATH / Path("connection-status/unprotected.svg")))
        )
        self._fastest_icon = Gio.FileIcon.new(
            Gio.File.new_for_path(str(icons.ICONS_PATH / Path("connection-status/fastest.svg")))
        )
        ...
        self._status_icon = Gtk.Image.new_from_gicon(self._unprotected_icon)
    ...
    def connection_status_update(self, connection_state: states.State):
        ...
        if connecting or (disconnecting and reconnecting):
            self._show_spinner()
            self._set_status_title(_("Connecting..."), None)
        elif connected:
            self._show_icon(self._protected_icon, 24)
            self._set_status_title(_("Protected"), "protected")
            if self._split_tunneling_enabled:
                self._notifications.show_info_message(message=_(SPLIT_TUNNELING_APP_RESTART_MESSAGE))
        elif disconnecting and not reconnecting:
            self._show_spinner()
            self._set_status_title(_("Disconnecting..."), None)
        elif error:
            self._show_icon(None)
            self._set_status_title(_("Connection error"), None)
            self._on_connection_error(connection_state)
        elif disconnected:
            self._show_icon(self._unprotected_icon, 24)
            self._set_status_title(_("Unprotected"), "unprotected")

        self._update_connection_details(connection_state.context.connection, disconnected and not reconnecting)
    ...
    def _show_icon(self, icon: Optional[Gio.Icon], pixel_size: int = 24):
        self._status_spinner.stop()
        self._status_spinner.set_visible(False)
        if icon is not None:
            self._status_icon.set_from_gicon(icon)
            self._status_icon.set_pixel_size(pixel_size)
    ...
    def _update_connection_details(self, connection, disconnected: bool):
        ...
        if disconnected:
            self._connection_details_icon.set_from_gicon(self._fastest_icon)
            self._connection_details_icon.set_pixel_size(24)
            is_free = self._controller.user_tier == TierEnum.FREE
            if is_free:
                self._connection_details_title.set_text(_("Fastest free server"))
                self._connection_details_subtitle.set_text(_("Auto-selected from free locations"))
            else:
                self._connection_details_title.set_text(_("Fastest country"))
            ...
            new_connection_details_icon = self._build_connection_details_icon(logical_server, is_secure_core)
            ...
            if is_secure_core:
                self._connection_details_subtitle.set_label(
                    _("Via {entry_country}").format(entry_country=logical_server.entry_country_name)
                )
            else:
                self._connection_details_subtitle.set_label(f"{logical_server.location} - {server_name}")
