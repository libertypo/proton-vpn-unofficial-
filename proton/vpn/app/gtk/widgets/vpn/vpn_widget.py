...
class VPNWidget(Gtk.Box):
    ...
    def __init__(self, controller: Controller, main_window: "MainWindow", notifications=Notifications):
        ...
        self.connection_status_widget = VPNConnectionStatusWidget(controller, notifications)
        ...
        self._connected_signals.append(
            (
                safe_signal_connect(self.server_list_widget, "ui-updated", self._on_server_list_updated),
                self.server_list_widget,
            )
        )
        main_window.add_keyboard_shortcut(
            target_widget=self.search_widget, target_signal="request_focus", shortcut="<Control>f"
        )
        ...
        self._connected_signals.append(
            (
                safe_signal_connect(
                    self.search_results_widget, "result-chosen", self.server_list_widget.focus_on_entry
                ),
        ...
        self._connected_signals.append(
            (
                safe_signal_connect(self.search_results_widget, "result-chosen", self._reset_search_on_result_chosen),
                self.search_results_widget,
            )
        )
        ...
        signal_id = safe_signal_connect(self, "connection-state-changed", self._broadcast_connection_state)
    ...
    def status_update(self, connection_state: State):
        ...
        logger.debug("VPN widget received connection status update: %s.", type(connection_state).__name__)

        GLib.idle_add(self.emit, "connection-state-changed", connection_state)

    def _on_refresher_enabled(self, future: Future):
        try:
            future.result()
        except (RuntimeError, OSError, TimeoutError) as error:
            logger.warning(
                "Refresher initialization failed: %s",
                error,
                category="APP",
                subcategory="VPN",
                event="RESULT",
    ...
    def display(self, user_tier: int, server_list: ServerList):
        ...
        reconnector = self._controller.reconnector
        if reconnector is None:
            raise RuntimeError("VPN reconnector is not initialized")
        reconnector.enable()
    ...
    def unload(self):
        ...
        for widget in [self.connection_status_widget, self.quick_connect_widget, self.server_list_widget]:
