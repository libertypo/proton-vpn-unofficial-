...
class SecureCoreRow(Gtk.Box):
    ...
    def display(
        ...
        connect_button_tooltip = (
            ...
            if upgrade_required
            else f"Connect to {exit_country_name} via Secure Core"
        )
    ...
    def _add_server_rows(self) -> None:
        ...
        def display_server_row(server_row: RowContent, server: LogicalServer) -> None:
            ...
            row_data = RowViewModel(
                ...
                connect_button_tooltip=(
                    f"Upgrade to connect to {server.exit_country_name} via {server.entry_country_name}"
                    if upgrade_required
                    else f"Connect to {server.exit_country_name} via {server.entry_country_name}"
                ),
            )
