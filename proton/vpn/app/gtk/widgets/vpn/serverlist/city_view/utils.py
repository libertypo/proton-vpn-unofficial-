...
def make_connect_callback(controller: Controller, servers: list, user_tier: int) -> Callable[[], None]:
    """Returns a callback that connects to the fastest available server in the given list."""

    def on_connect():
        fastest = ServerList.get_fastest_server(ServerList.get_available_servers(servers=servers, user_tier=user_tier))
