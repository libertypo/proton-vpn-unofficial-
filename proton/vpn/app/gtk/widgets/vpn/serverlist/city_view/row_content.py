...
class RowContent(Gtk.Box):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        ...
        self._hover_button_content = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
    ...
    def display(self, row_data: RowViewModel):
        ...
        if row_data.toggable:
            ...
            signal_id = safe_signal_connect(self.toggle_button, "clicked", self._on_toggle_button_clicked)
    ...
    def _show_hover_stack(self):
        ...
        if self._row_data.upgrade_required:
            ...
            signal_id = safe_signal_connect(self.action_button, "clicked", self._on_upgrade_button_clicked)
            self.add_css_class("dimmed")
        else:
            self._hover_action_label.set_text("Connect")
            signal_id = safe_signal_connect(self.action_button, "clicked", self._on_connect_button_clicked)
    ...
    def _configure_connect_button(self, tooltip: str):
        ...
        add_accessibility(self.action_button, Gtk.AccessibleRelation.LABELLED_BY, self._accessibility_label)
        if self._feature_icons:
            add_accessibility(self.action_button, Gtk.AccessibleRelation.DESCRIBED_BY, self._feature_icons)
    ...
    def expanded(self, value: bool):
        ...
        self.toggle_button.set_child(self._expanded_img if self.expanded else self._collapsed_img)
        if self._row_data is None or self._row_data.toggle_button_tooltips is None:
            return

        tooltip_text = (
            self._row_data.toggle_button_tooltips[1] if self.expanded else self._row_data.toggle_button_tooltips[0]
        )
