...
class ServerListWidget(Gtk.ScrolledWindow):
    ...
    def _display_country_rows(self, server_list: ServerList):
        ...
        countries = server_list.group_by_country(group_by_location=True, include_free_servers=free_user)
        if free_user:
        ...
        expanded_groups_per_country = {
            country_row.country_code.lower(): set(
                location_row.label.lower()
                for location_row in (
                    country_row.location_rows + ([country_row.secure_core_row] if country_row.secure_core_row else [])
                )
        ...
        def display_country_row(row, country):
            ...
            row.display(self._controller, country, self._user_tier, expanded=expanded, expanded_groups=expanded_groups)

        sync_rows_with_model_items(
            countries, self._country_rows, self._country_rows_container, CountryRow, display_country_row
        )
    ...
    def _on_server_list_update(self):
        ...
        logger.info("Full server list widget update completed in %.2f seconds.", time.time() - start)

    def _on_server_loads_update(self):
        start = time.time()
        self.display(self._user_tier, self._controller.server_list)
        logger.info("Partial server list widget update completed in %.2f seconds.", time.time() - start)
