...
_LEGEND_ITEMS: list[LegendItem] = [
    LegendItem(
        ...
        _("This technology allows Proton VPN to provide higher speed and security in difficult-to-reach countries."),
    ...
    LegendItem(
        P2PIcon,
        _("P2P/BitTorrent"),
        _("These servers give the best performance for BitTorrent and file sharing."),
        "https://protonvpn.com/features/p2p-support",
    ),
    LegendItem(
        TORIcon,
        _("Tor"),
        _("Connect to a Tor server to access hidden services and onion sites using any browser."),
...
class FeatureLegendPopover(Gtk.Popover):
    ...
    def _build_header(self) -> Gtk.Box:
        ...
        title = Gtk.Label(label=_("Features"))
    ...
    def _build_feature_rows(self) -> Gtk.Box:
        ...
        for item in _LEGEND_ITEMS:
            content.append(self._build_legend_row(item.icon_cls, item.heading, item.description, item.learn_more_url))
        return content
    ...
    def _build_learn_more_link(url: str) -> Gtk.LinkButton:
        ...
        link = Gtk.LinkButton(uri=url, label=_("Learn more"))
...
class ServerListHeaderRow(Gtk.Box):
    ...
    def __init__(self):
        ...
        info_icon = Gtk.Image.new_from_gicon(
            Gio.FileIcon.new(Gio.File.new_for_path(str(icons.ICONS_PATH / Path("info.svg"))))
        )
    ...
    def _format_count(self, count: int) -> str:
        ...
        return _("All countries ({count})").format(count=count)
