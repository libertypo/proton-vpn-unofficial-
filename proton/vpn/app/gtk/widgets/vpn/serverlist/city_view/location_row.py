...
class LocationRow(Gtk.Box):
    ...
    def display(self, controller: Controller, location: Location, user_tier: int, expanded: bool = False):
        ...
        row_data = RowViewModel(
            ...
            connect_button_tooltip=(
                f"Upgrade to connect to {location.name}" if upgrade_required else f"Connect to {location.name}"
            ),
    ...
    def _add_server_rows(self):
        ...
        def display_server_row(server_row, server):
            ...
            row_data = RowViewModel(
                ...
                connect_button_tooltip=(
                    f"Upgrade to connect to {server.name}" if upgrade_required else f"Connect to {server.name}"
                ),
            )
            server_row.display(row_data)

        sync_rows_with_model_items(
            list(servers), self._server_rows, self._expandable_row.container, RowContent, display_server_row
        )
