...
def _set_image_from_pixbuf(image: Gtk.Image, pixbuf: GdkPixbuf.Pixbuf):
...
class UnderMaintenanceIcon(Gtk.Image):
    ...
    def __init__(self, widget_under_maintenance: Optional[str] = None):
        ...
        _set_image_from_pixbuf(self, pixbuf)
...
class SecureCoreIcon(Gtk.Image):
    ...
    def __init__(
        ...
        if entry_country_name and exit_country_name:
            help_text = f"Secure core server that connects to {exit_country_name} through {entry_country_name}."
        else:
...
class DoubleFlagIcon(Gtk.Image):
    ...
    def __init__(self, exit_country_code: str, entry_country_code: str):
        ...
        scaled = composite.scale_simple(
            CountryFlagIcon._FLAG_WIDTH, CountryFlagIcon._FLAG_HEIGHT, GdkPixbuf.InterpType.BILINEAR
        )
        if scaled is None:
            raise RuntimeError("Failed to scale composited flag icon")
        _set_image_from_pixbuf(self, scaled)
...
class CountryFlagIcon(Gtk.Image):
    ...
    def __init__(self, country_code: str):
        ...
        try:
            pixbuf = icons.get(
                Path("flags") / f"{country_code.lower()}.svg", width=self._FLAG_WIDTH, height=self._FLAG_HEIGHT
            )
        except ValueError:
