...
class PortForwardRevealer(Gtk.Revealer):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, notifications: Notifications, port_forward_widget: Optional["PortForwardWidget"] = None):
        super().__init__()
        self._port_forward_widget = port_forward_widget or PortForwardWidget(notifications)
        self.set_child(self._port_forward_widget)
        safe_signal_connect(self._port_forward_widget, "update-visibility", self._on_update_port_forwarding_visibility)
...
class PortForwardWidget(Gtk.Box):
    ...
    ACTIVE_PORT_LABEL = _("Active port:")
    TOOLTIP_LABEL = _("Copy port number")
    ...
    def _build_ui(self) -> None:
        ...
        active_port_icon = Gtk.Image.new_from_gicon(
            Gio.FileIcon.new(Gio.File.new_for_path(str(icons.ICONS_PATH / Path("connection-status/active-port.svg"))))
        )
        active_port_icon.set_pixel_size(12)
        ...
        copy_icon = Gtk.Image.new_from_gicon(
            Gio.FileIcon.new(Gio.File.new_for_path(str(icons.ICONS_PATH / Path("copy.svg"))))
        )
        ...
        self._copied_popover.set_child(Gtk.Label(label=_("Copied!")))
    ...
    def on_new_state(self, connection_state: states.State):
        ...
        self._update_visibility(
            connection_state.forwarded_port, reveal_child=isinstance(connection_state, states.Connected)
        )
        ...
        if new_port and (new_port != self._current_forwarded_port):
            if not is_focus:
                self._notifications.show_gnome_notification(
                    title=_("Port forwarding"), description=_("Active port is {port}").format(port=new_port)
                )
    ...
    def _on_button_press(self, _: "PortForwardWidget"):
        ...
        if isinstance(self.get_root(), Gtk.Window) and self._copied_popover is not None:
            ...
            self._cancel_pending_popdowns()
            self._pending_sources.append(GLib.timeout_add(1500, self._on_popdown_timer))
