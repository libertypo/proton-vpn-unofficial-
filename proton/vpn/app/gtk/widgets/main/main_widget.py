...
class MainWidget(Gtk.Overlay):
    ...
    SESSION_EXPIRED_ERROR_MESSAGE = "Your session has expired. Please sign in again."
    ...
    def __init__(
        ...
        self._notifications = notifications or Notifications(main_window, NotificationBar())
        ...
        safe_signal_connect(self._main_window.header_bar.menu, "user-logged-out", self._on_user_logged_out)
    ...
    def initialize_visible_widget(self):
        ...
        if self._controller.user_logged_in:
            ...
            connect_once(self.vpn_widget, "vpn-widget-ready", self._controller.run_startup_actions)
        else:
            self._display_login_widget()

    def show_error_message(self, error_message: str, blocking: bool = False, error_title: Optional[str] = None):
        """
        ...
        """
        if blocking:
            self.notifications.show_error_dialog(error_message, error_title or self.ERROR_DIALOG_PRIMARY_TEXT)
        else:
    ...
    def on_session_expired(self):
        ...
        self.notifications.show_error_dialog(
            title=self.SESSION_EXPIRED_ERROR_TITLE, message=self.SESSION_EXPIRED_ERROR_MESSAGE
        )
        self._display_login_widget()
    ...
    def _create_login_widget(self) -> LoginWidget:
        login_widget = LoginWidget(self._controller, self.notifications, self._overlay_widget, self._main_window)
    ...
    def _create_vpn_widget(self) -> VPNWidget:
        vpn_widget = VPNWidget(
            controller=self._controller, main_window=self._main_window, notifications=self.notifications
        )
        safe_signal_connect(vpn_widget, "vpn-widget-ready", self._hide_overlay_widget)
        signal_id = safe_signal_connect(
            vpn_widget, "connection-state-changed", self._change_gradient_on_connection_state_change
        )
    ...
    def _display_vpn_widget(self):
        ...
        self._overlay_widget.show(DefaultLoadingWidget("Loading app..."))
