...
class ProtonReport(Gtk.Box):
    ...
    def __init__(self):
        ...
        self.texture = Gdk.Texture.new_from_file(Gio.File.new_for_path(str(icons.ICONS_PATH / Path("NPS/report.svg"))))
        self.texture_success = Gdk.Texture.new_from_file(
            Gio.File.new_for_path(str(icons.ICONS_PATH / Path("NPS/report-success.svg")))
        )
...
class NPSSurveyModal(Gtk.Window):
    ...
    def __init__(self, controller: Controller, submit_handler: NPSSubmitHandler, dismiss_handler: NPSDismissHandler):
        super().__init__()
        self.set_modal(True)
        ...
        self._dismiss_handler_id = safe_signal_connect(self, "close-request", self._on_close_request)
    ...
    def _build_feedback_text(self):
        ...
        self._char_counter_label = Gtk.Label(label=f"0/{NPSSurveyResponse.COMMENT_CHAR_MAX_LENGTH}")
        ...
        safe_signal_connect(self._feedback_text_view.get_buffer(), "changed", self._on_text_changed)
    ...
    def _on_text_changed(self, _):
        ...
        self._char_counter_label.set_label(f"{count}/{NPSSurveyResponse.COMMENT_CHAR_MAX_LENGTH}")
    ...
    def state(self) -> "NPSSurveyModal.State":
        ...
        if current_state is None:
            raise RuntimeError("Survey state is not initialized")
