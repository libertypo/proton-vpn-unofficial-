...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def __init__(
        ...
        notifications = notifications or Notifications(main_window=self, notification_bar=NotificationBar())

        self.header_bar = header_bar or HeaderBar(
            controller=controller, main_window=self, overlay_widget=self._overlay_widget
        )
        self.set_titlebar(self.header_bar)
        self.set_title("Proton VPN")

        self.main_widget = main_widget or MainWidget(
            controller=controller, main_window=self, notifications=notifications, overlay_widget=self._overlay_widget
        )
    ...
    def configure_close_button_to_hide_window(self):
        ...
        return safe_signal_connect(self, "close-request", self._on_close_button_clicked_then_hide_window)
    ...
    def configure_close_button_to_trigger_quit_menu_entry(self):
        ...
        # Handle the event emitted when the user tries to close the window.
        return safe_signal_connect(self, "close-request", self._on_close_button_clicked_then_click_quit_menu_entry)
    ...
    def create_nps_survey_modal(self) -> NPSSurveyModal:
        ...
        def submit_nps_survey_feedback(score: int, comments: str):
            nps_user_response = NPSSurveyResponse(
                user_score=score, user_comments=comments, response_type=NPSSurveyResponse.ResponseType.SUBMIT
            )
        ...
        def dismiss_nps_survey():
            nps_user_response = NPSSurveyResponse(response_type=NPSSurveyResponse.ResponseType.DISMISS)
            future = self._controller.submit_nps_survey_response(nps_user_response)
            future.add_done_callback(self._on_nps_submission_result)

        return NPSSurveyModal(
            self._controller, submit_handler=submit_nps_survey_feedback, dismiss_handler=dismiss_nps_survey
        )
    ...
    def _on_nps_submission_result(self, future: Future):
        try:
            ...
            def _reraise_on_main_thread(exc=exc):
                raise NPSError("Unexpected error submitting NPS survey response.") from exc

            GLib.idle_add(_reraise_on_main_thread)
        except (RuntimeError, OSError, TimeoutError) as exc:
            logger.warning("NPS survey submission failed: %s", exc)
