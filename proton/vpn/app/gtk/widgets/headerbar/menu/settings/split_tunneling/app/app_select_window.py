...
from proton.vpn.app.gtk.widgets.headerbar.menu.settings.split_tunneling.app.data_structures import (
    AppData,
    AppRowWithCheckbox,
)
    ...
    def _build_ui(self, _: Gtk.Window):
        for app_data in self._installed_apps:
            self.content_container.append(
                AppRowWithCheckbox.build(app_data=app_data, checked=bool(app_data.executable in self._stored_apps))
            )
        ...
        safe_signal_connect(connect_button, "clicked", self._on_done_button_clicked)
