...
from proton.vpn.app.gtk.widgets.headerbar.menu.settings.split_tunneling.app.app_select_window import AppSelectionWindow
...
class AppBasedSplitTunnelingSettings(Gtk.Box):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        setting_path_name_template: str = "settings.features.split_tunneling.[mode].app_paths",
        ...
        self._mode = mode
        self._stored_apps = stored_apps if stored_apps is not None else self._get_settings()
        self._installed_apps = installed_apps if installed_apps is not None else get_all_installed_apps()
        self._selected_app_list = (
            selected_app_list if selected_app_list is not None else SelectedAppList(self._get_selected_app_list())
        )
        ...
        safe_signal_connect(self._selected_app_list, "app-list-refreshed", self._on_app_list_refreshed)
    ...
    def _on_clicked_add(self, _: Gtk.Button):
        ...
        safe_signal_connect(self._add_app_window, "app-selection-completed", self._on_app_selection_completed)
        safe_signal_connect(self._add_app_window, "unrealize", self._on_add_app_window_unrealize)
    ...
    def _on_app_selection_completed(self, _: AppSelectionWindow, selected_apps: list[AppData]):
    ...
    def _on_app_removed(self, _: SelectedAppList, app_data: AppData):
        self._stored_apps = [app_exec for app_exec in self._stored_apps if app_exec != app_data.executable]
