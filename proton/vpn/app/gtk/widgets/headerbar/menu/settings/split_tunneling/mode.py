...
EXCLUDE_MODE = ModeData(
    title="Exclude mode",
    description="Allow selected apps to connect without VPN protection.",
    mode=SplitTunnelingMode.EXCLUDE,
)

INCLUDE_MODE = ModeData(
    title="Include mode",
    description="Only selected apps connect with VPN protection. All other traffic is unprotected.",
...
class ModeRadioButton(Gtk.Box):
    ...
    def __init__(self, mode_data: ModeData):
        ...
        self._title_label = SettingName(_(mode_data.title), bold=True)

        self._description_label = SettingDescription(_(mode_data.description))
