...
RECONNECT_MESSAGE = "Please establish a new VPN connection for changes to take effect."


class ActiveIdDropDown(Gtk.DropDown):
    """DropDown wrapper that preserves the active_id API used by settings widgets."""
    ...
    def __init__(self) -> None:
        ...
        self._updating_active_id = False
        self._active_id: Optional[str] = None
        self.connect("notify::selected", self._on_selected_changed)

    def append(self, value: str, display: str) -> None:
        self._option_ids.append(value)
        self._model.append(display)

    def remove_all(self) -> None:
        item_count = self._model.get_n_items()
        while item_count > 0:
            self._model.remove(0)
            item_count -= 1
        self._option_ids = []
        self._set_active_id_internal(None)

    def get_active_id(self) -> Optional[str]:
        return self._active_id

    def set_active_id(self, value: Optional[str]) -> None:
        if value is None:
            self._set_active_id_internal(None)
            self.set_selected(Gtk.INVALID_LIST_POSITION)
            return

        index = self._find_index_for_id(value)
        if index is None:
            self._set_active_id_internal(None)
            self.set_selected(Gtk.INVALID_LIST_POSITION)
            return

        self._set_active_id_internal(value)
        self.set_selected(index)

    def _find_index_for_id(self, value: str) -> Optional[int]:
        try:
            return self._option_ids.index(value)
        except ValueError:
            return None
    ...
    def _on_selected_changed(
        ...
        if self._updating_active_id:
            return

        selected = self.get_selected()
        if selected == Gtk.INVALID_LIST_POSITION:
            self._set_active_id_internal(None)
        elif 0 <= selected < len(self._option_ids):
            self._set_active_id_internal(self._option_ids[selected])
        else:
            self._set_active_id_internal(None)

        self.emit("changed")

    def _set_active_id_internal(self, value: Optional[str]) -> None:
        if self._active_id == value:
            return
        self._updating_active_id = True
...
class ToggleWidget(Gtk.Grid):  # pylint: disable=too-many-instance-attributes
    ...
    def set_tooltip(self, tooltip_text: str):
        ...
        if self._display_tooltip_only_on_active_connection and not self._controller.connection_disconnected:
    ...
    def _is_upgrade_required(self) -> bool:
        ...
        return is_upgrade_required(self._requires_subscription_to_be_active, self._controller.user_tier)
...
class ComboboxWidget(Gtk.Grid):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        title: str,
        setting_name: str,
        combobox_options: List[Tuple[Union[str, int], str]],
    ...
    def _build_combobox(self) -> ActiveIdDropDown:
        combobox = ActiveIdDropDown()
        ...
        if self._callback:
            self._changed_callback_id = safe_signal_connect(combobox, "changed", self._callback)
        else:
            self._changed_callback_id = safe_signal_connect(combobox, "changed", self._on_combobox_change)
    ...
    def _on_combobox_change(self, combobox):
        active_id = combobox.get_active_id()
        if not active_id:
            return
        self.save_setting(active_id)
...
class ConflictableComboboxWidget(ComboboxWidget):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        setting_name: str,
        combobox_options: List[Tuple[Union[str, int], str]],
        do_set: Callable[[ComboboxWidget, str], None],
    ...
    def _on_conflictable_combobox_change(self, combobox_text_widget):
        new_value = combobox_text_widget.get_active_id()

        if conflict := self._controller.setting_attr_has_conflict(self._setting_name, new_value):
            parent = _get_parent_window(self)
            if parent is None:
                logger.warning("Unable to show conflict dialog: missing parent window")
