...
def _get_flatpak_executable(app: Gio.AppInfo) -> str:
    ...
    if not result:
        raise DesktopFileParsingError("Could not parse flatpack executable string from: {command_line}")
...
def _get_native_app_executable(app: Gio.AppInfo) -> str:
    ...
    if not executable:
        raise DesktopFileParsingError("Could not get path from command: {command}")
...
def get_all_installed_apps() -> list[AppData]:
    ...
    for app in Gio.AppInfo.get_all():
        ...
        try:
            ...
            logger.warning("Could not get app executable for %s", app.get_id(), exc_info=True)
            continue

        icon = get_app_icon(app)

        app_list.append(AppData(name=html.escape(app.get_display_name()), executable=executable, icon_name=icon))
