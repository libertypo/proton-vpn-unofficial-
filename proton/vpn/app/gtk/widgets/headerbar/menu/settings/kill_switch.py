...
class KillSwitchWidget(ConflictableToggleWidget, ReactiveSetting):  # noqa pylint: disable=too-many-instance-attributes,too-few-public-methods
    ...
    KILLSWITCH_STANDARD_DESCRIPTION = "Automatically disconnect from the internet if VPN connection is lost."
    KILLSWITCH_ADVANCED_DESCRIPTION = (
        "Only allow internet access when connected to Proton VPN. "
        "Advanced kill switch will remain active even when you restart your device."
    )
    SETTING_NAME = "settings.killswitch"

    def __init__(
        self, controller: Controller, gtk: Optional[ModuleType] = None, conflict_resolver: Optional[Callable] = None
    ):
        super().__init__(
    ...
    def _build_standard_killswitch(self) -> Gtk.Grid:
        ...
        main_standard_container.attach(SettingDescription(self.KILLSWITCH_STANDARD_DESCRIPTION), 1, 1, 1, 1)

        safe_signal_connect(self.standard_radio_button, "toggled", self._on_standard_radio_button_toggle)
    ...
    def _build_advanced_killswitch(self) -> Gtk.Grid:
        ...
        main_advanced_container.attach(SettingDescription(self.KILLSWITCH_ADVANCED_DESCRIPTION), 1, 1, 1, 1)

        safe_signal_connect(self.advanced_radio_button, "toggled", self._on_advanced_radio_button_toggle)
    ...
    def _handle_toggle(self, radio_button: Gtk.CheckButton, new_value: int):
        ...
        if revealer is None:
            raise RuntimeError("Kill switch revealer not initialized")
    ...
    def _do_set(self, _toggle, new_value: bool):
        revealer = self.revealer
        standard_radio_button = self.standard_radio_button
        if revealer is None or standard_radio_button is None:
            raise RuntimeError("Kill switch controls are not initialized")
        revealer.set_reveal_child(new_value)

        if not new_value:
            standard_radio_button.set_active(True)
        ...
        if not standard_radio_button.get_active():
            standard_radio_button.set_active(True)
            return
    ...
    def get_killswitch_state(self) -> KillSwitchSettingEnum:
        ...
        standard_radio_button = self.standard_radio_button
        advanced_radio_button = self.advanced_radio_button
        if standard_radio_button is None or advanced_radio_button is None:
            raise RuntimeError("Kill switch controls are not initialized")
        if self.switch.get_active():
            if standard_radio_button.get_active():
                return KillSwitchSettingEnum.ON
            if advanced_radio_button.get_active():
    ...
    def set_killswitch_state(self, state: KillSwitchSettingEnum):
        ...
        if state == KillSwitchSettingEnum.ON:
            ...
            self.switch.set_active(True)
            advanced_radio_button.set_active(True)
        else:
            self.switch.set_active(False)
