...
class FeatureSettings(BaseCategoryContainer, ReactiveSettingContainer):  # noqa: E501 # pylint: disable=line-too-long, too-many-instance-attributes
    ...
    NETSHIELD_DESCRIPTION = "Protect yourself from ads, malware, and trackers on websites and apps."
    ...
    SWITCH_KILLSWITCH_IF_CONNECTION_ACTIVE_DESCRIPTION = (
        "Kill switch selection is disabled while VPN is active. Disconnect to make changes."
    )
    ...
    def build_netshield(self):
        ...
        """
        netshield_options = [
            (str(NetShield.NO_BLOCK.value), _("Off")),
            (str(NetShield.BLOCK_MALICIOUS_URL.value), _("Block Malware")),
            (str(NetShield.BLOCK_ADS_AND_TRACKING.value), _("Block ads, trackers and malware")),
        ]
        self.netshield = ComboboxWidget(
            controller=self._controller,
            title=_(self.NETSHIELD_LABEL),
            description=_(self.NETSHIELD_DESCRIPTION),
    ...
    def _on_netshield_combobox_changed(self, combobox: Gtk.ComboBoxText):
        ...
        if active_id is None:
            return
        netshield_widget = self.netshield
        if netshield_widget is None:
            raise RuntimeError("NetShield widget is not initialized")
        netshield = int(active_id)
        netshield_widget.save_setting(netshield)
    ...
    def build_port_forwarding(self):
        ...
        self.port_forwarding = ToggleWidget(
            controller=self._controller,
            title=_(self.PORT_FORWARDING_LABEL),
            description=_(self.PORT_FORWARDING_DESCRIPTION_LEARN_MORE),
    ...
    def _on_dialog_button_click(self, confirmation_dialog: ConfirmationDialog, response_type: int):
    ...
    def on_custom_dns_setting_changed(self, custom_dns_widget: CustomDNSWidget, custom_dns_enabled: int):
        """Called on custom DNS setting change (conflict resolution)"""
        netshield_widget = self.netshield
        if netshield_widget is None:
            raise RuntimeError("NetShield widget is not initialized")
        netshield_disabled = int(netshield_widget.get_setting()) == NetShield.NO_BLOCK

        if not custom_dns_enabled or netshield_disabled:
            self._settings_window.notify_user_with_reconnect_message(only_notify_on_active_connection=True)
            return
    ...
    def _build_dialog_content(self):
        ...
        question = Gtk.Label(label=_("Enable Custom DNS ?"))
        question.set_halign(Gtk.Align.START)

        clarification = Gtk.Label(label=_("This will disable Netshield."))
        clarification.set_halign(Gtk.Align.START)
        clarification.add_css_class("dim-label")

        learn_more = Gtk.Label(
            label='<a href="https://protonvpn.com/support/custom-dns#netshield">{}</a>'.format(_("Learn more"))
        )
