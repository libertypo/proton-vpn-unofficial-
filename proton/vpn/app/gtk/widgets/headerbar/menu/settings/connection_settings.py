...
class ProtocolComboboxWidget(ConflictableComboboxWidget, ReactiveSetting):
    ...
    def on_settings_changed(self, settings):
        ...
        """
        if self.combobox.get_active_id() != settings.protocol:
    ...
    def _repopulate_combobox(self, protocols):
        ...
        self._combobox_options = [(str(protocol.protocol), protocol.ui_protocol) for protocol in protocols]
        with self.pause_callback():
            self.combobox.remove_all()
            for protocol_id, protocol_label in self._combobox_options:
                self.combobox.append(protocol_id, protocol_label)
    ...
    def _on_protun_checkbox_toggled(self, checkbox):
        ...
        if protocols:
            self.combobox.set_active_id(str(protocols[0].protocol))
...
class ConnectionSettings(BaseCategoryContainer, ReactiveSettingContainer):  # noqa: E501 # pylint: disable=line-too-long, too-many-instance-attributes
    ...
    VPN_ACCELERATOR_DESCRIPTION = (
        "Increase your connection speed by up to 400% with performance enhancing technologies."
    )
    MODERATE_NAT_LABEL = "Moderate NAT"
    MODERATE_NAT_DESCRIPTION = (
    ...
    SWITCH_PROTOCOL_IF_CONNECTION_ACTIVE_DESCRIPTION = (
        "Protocol selection is disabled while VPN is active. Disconnect to make changes."
    )
    IPV6_LABEL = "IPv6"
    IPV6_DESCRIPTION = "Tunnels IPv6 traffic through the VPN. Can enhance compatibility with IPv6 networks."
