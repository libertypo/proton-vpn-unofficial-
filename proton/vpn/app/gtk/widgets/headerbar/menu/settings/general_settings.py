...
class TrayPinnedServersWidget(EntryWidget):
    ...
    TRAY_PINNED_SERVERS_DESCRIPTION = (
    ...
    def __init__(self, controller: Controller, tray_indicator: Optional["TrayIndicator"] = None):
        super().__init__(
            ...
            title=_(self.TRAY_PINNED_SERVERS_LABEL),
            description=_(self.TRAY_PINNED_SERVERS_DESCRIPTION),
...
def default_file_browser(packet_capture_widget: "PacketCaptureWidget"):
    ...
    def _on_browse_clicked(_button):
        ...
        dialog.set_title(_("Select packet capture directory"))
        dialog.set_accept_label(_("_Select"))
...
class PacketCaptureWidget(EntryWidget):
    ...
    def __init__(
        self,
        controller: Controller,
        file_browser: Callable[["PacketCaptureWidget"], Optional[Callable]] = default_file_browser,
    ):
    ...
    def _build_ui(self):
        ...
        self._start_stop_button = Gtk.Button(label=_("Start"))
    ...
    def _on_unrealize(self, _widget):
        if self._capturing:
            self._controller.executor.submit(self._controller.current_connection.stop_packet_capture).add_done_callback(
                lambda f: GLib.idle_add(self._set_capturing, False)
            )
    ...
    def _set_capturing(self, capturing: bool):
        ...
        self._start_stop_button.set_label(_("Stop") if capturing else _("Start"))
        if capturing:
...
class GeneralSettings(BaseCategoryContainer):  # pylint: disable=too-many-instance-attributes
    ...
    CONNECT_AT_APP_STARTUP_DESCRIPTION = (
    ...
    START_APP_MINIMIZED_DESCRIPTION = "When enabled, the app starts minimized to the tray."
    ANONYMOUS_CRASH_REPORTS_LABEL = "Share anonymous crash reports"
    ANONYMOUS_CRASH_REPORTS_DESCRIPTION = (
    ...
    def on_settings_changed(self, settings):
        ...
        if self._packet_capture_widget is not None:
            self._packet_capture_widget.set_visible(self._protocol_supports_packet_capture(settings.protocol))
    ...
    def build_connect_at_app_startup(self):
        ...
        self._connect_at_startup_entry = EntryWidget(
            ...
            title=_(self.CONNECT_AT_APP_STARTUP_LABEL),
            description=_(self.CONNECT_AT_APP_STARTUP_DESCRIPTION),
    ...
    def build_start_app_minimized(self):
        ...
        self._start_app_minimized_toggle = ToggleWidget(
            controller=self._controller,
            title=_(self.START_APP_MINIMIZED_LABEL),
            description=_(self.START_APP_MINIMIZED_DESCRIPTION),
    ...
    def build_tray_pinned_servers(self):
        ...
        self.append(TrayPinnedServersWidget(controller=self._controller, tray_indicator=self._tray_indicator))
    ...
    def build_anonymous_crash_reports(self):
        ...
        self._anonymous_crash_reports_toggle = ToggleWidget(
            controller=self._controller,
            title=_(self.ANONYMOUS_CRASH_REPORTS_LABEL),
            description=_(self.ANONYMOUS_CRASH_REPORTS_DESCRIPTION),
    ...
    def build_beta_upgrade(self):
        ...
        # can_early_access_be_displayed() has now populated the packages cache.
        # The widget was constructed with enabled=False to avoid blocking the
        # main thread during init, so set the correct switch on/off state now
        # that the cached result is available (get_setting() is instant here).
        early_access.switch.set_active(early_access.get_setting())
