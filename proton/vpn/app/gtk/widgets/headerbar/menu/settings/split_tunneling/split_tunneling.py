...
class SplitTunnelingSettings(Gtk.Box):
    ...
    def __init__(
        ...
        self._split_tunneling_mode = split_tunneling_mode or SplitTunnelingModeSetting(controller=self._controller)
        self._split_tunneling_apps = split_tunneling_apps or AppBasedSplitTunnelingSettings(
            controller=self._controller, mode=self._split_tunneling_mode.mode, gtk=self.gtk
        )
        self._split_tunneling_ips = split_tunneling_ips or IpBasedSplitTunnelingSettings(
    ...
    def build(
        ...
        settings_container = SplitTunnelingSettings(
            controller, split_tunneling_mode, split_tunneling_apps, split_tunneling_ips, gtk
        )
...
class SplitTunnelingToggle(ConflictableToggleWidget, ReactiveSetting):
    ...
    DESCRIPTION = "Customize your connection by deciding which apps are protected by VPN."
    ...
    def _do_set(self, _toggle, new_value: bool):
        ...
        if revealer is None:
            raise RuntimeError("Split tunneling revealer not initialized")
    ...
    def _build_and_add_split_tunneling_settings(self):
        self._settings_container = SplitTunnelingSettings(controller=self._controller, gtk=self.gtk)
