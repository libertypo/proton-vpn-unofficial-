...
class CustomDNSManager(Gtk.Box):  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        self, controller: Controller, gtk: Optional[ModuleType] = None, custom_dns_list: Optional[CustomDNSList] = None
    ):
        super().__init__(orientation=Gtk.Orientation.VERTICAL)
        self.set_spacing(15)
        ...
        label = self.gtk.Label(label=_("Add new server"))
        ...
        safe_signal_connect(self._custom_dns_list, "dns-ip-removed", self._on_dns_delete_clicked)
    ...
    def _build_entry_row(self) -> Gtk.Grid:
        ...
        self._add_button: Gtk.Button = self.gtk.Button(label=_("Add"))
    ...
    def _on_dns_add_clicked(self, _: Gtk.Button):
    ...
    def _notify_user_of_invalid_dns_entry(self, error_message_revealer: Gtk.Revealer):
        ...
        child.set_label(_(self.INVALID_IP_ERROR_MESSAGE))
...
class CustomDNSWidget(ToggleWidget):
    ...
    def __init__(self, controller: Controller, settings_window: Gtk.Window, gtk: Optional[ModuleType] = None):
        super().__init__(
    ...
    def _on_switch_button_toggle(self, _, new_value: bool, __):
        ...
        if revealer is None:
            raise RuntimeError("Custom DNS revealer not initialized")
    ...
    def _on_dialog_button_click(self, confirmation_dialog: ConfirmationDialog, response_type: int):
        ...
        if enable_netshield:
            ...
            if self._conflict_feature_settings is not None:
                netshield_widget = self._conflict_feature_settings.netshield
    ...
    def _build_dialog_content(self):
        ...
        question = self.gtk.Label(label=_("Enable Netshield ?"))
        question.set_halign(Gtk.Align.START)

        clarification = self.gtk.Label(label=_("This will disable custom DNS."))
