...
class LoginWidget(Gtk.Box):
    ...
    def __init__(
        ...
        self.login_stack = login_stack or LoginStack(self._controller, notifications, overlay_widget)
        safe_signal_connect(self.login_stack, "user-logged-in", self._on_user_logged_in)

        self.disable_killswitch = disable_killswitch_widget or DisableKillSwitchWidget(main_window)
        safe_signal_connect(self.disable_killswitch, "disable-killswitch", self._on_disable_killswitch)
    ...
    def reset(self):
        ...
        is_ks_permanent = self._controller.get_settings().killswitch == KillSwitchSettingEnum.PERMANENT
