...
class LoginForm(Gtk.Box):  # pylint: disable=R0902
    ...
    def __init__(self, controller: Controller, notifications: Notifications, overlay_widget: OverlayWidget):
        ...
        safe_signal_connect(self._password_entry, "changed", self._on_entry_changed)
        safe_signal_connect(self._username_entry, "changed", self._on_entry_changed)
    ...
    def _on_login_button_clicked(self, _):
        ...
        future = self._controller.login(username, password)
        future.add_done_callback(lambda future: GLib.idle_add(self._on_login_result, future))
    ...
    def _on_login_result(self, future: Future):
        try:
            ...
            logger.warning(error, category="APP", subcategory="LOGIN", event="RESULT", exc_info=True)
        ...
        if result.authenticated:
            ...
            self._notifications.show_error_message(self.INCORRECT_CREDENTIALS_MESSAGE)
            logger.warning(self.INCORRECT_CREDENTIALS_MESSAGE, category="APP", subcategory="LOGIN", event="RESULT")
            self.emit("login-error")
...
class LoginLinks(Gtk.Box):
    ...
    def __init__(self):
        ...
        create_account_link = Gtk.LinkButton(
            label="Create Account", uri="https://account.protonvpn.com/signup?ref=linux"
        )
        create_account_link.set_halign(Gtk.Align.START)
        create_account_link.set_hexpand(True)
        self.append(create_account_link)
        help_link = Gtk.LinkButton(label="Need Help?", uri="https://protonvpn.com/support")
