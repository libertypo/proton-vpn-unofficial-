...
class TwoFactorAuthStack(Gtk.Stack):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        # SecurityKeyForm
        self.security_key_form = security_key_form or SecurityKeyForm(controller, notifications, overlay_widget)
        safe_signal_connect(self.security_key_form, "two-factor-auth-successful", self._on_two_factor_auth_successful)
        safe_signal_connect(self.security_key_form, "two-factor-auth-cancelled", self._on_two_factor_auth_cancelled)
        ...
        safe_signal_connect(
            self.authenticator_app_form, "two-factor-auth-successful", self._on_two_factor_auth_successful
        )
        safe_signal_connect(
            self.authenticator_app_form, "two-factor-auth-cancelled", self._on_two_factor_auth_cancelled
        )
    ...
    def display_widget(self, widget: Union[AuthenticatorAppForm, SecurityKeyForm]):
        ...
        """
        if widget is not self.get_child_by_name("authenticator_app_form") and widget is not self.get_child_by_name(
            "security_key_form"
        ):
    ...
    def reset(self):
        ...
        if self._controller.fido2_available:
            self.add_titled(self.security_key_form, "security_key_form", self.SECURITY_KEY_FORM_TITLE)
            self.add_titled(self.authenticator_app_form, "authenticator_app_form", self.AUTHENTICATOR_APP_FORM_TITLE)
    ...
    def _on_two_factor_auth_cancelled(self, _):
        logger.info("2FA cancelled by user, signing out...", category="UI", subcategory="LOGIN-2FA", event="CLICK")
