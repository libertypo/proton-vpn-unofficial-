...
class SecurityKeyForm(Gtk.Box):  # pylint: disable=R0902
    ...
    DESCRIPTION_LABEL = "Insert the U2F or FIDO key linked to your Proton Account. " + LEARN_MORE_LINK
    ...
    INVALID_SECURITY_KEY_MESSAGE = (
        "Two-factor authentication failed: The security key you used is not linked to your Proton Account."
    )
    FIDO2_NOT_SUPPORTED_MESSAGE = "Two-factor authentication failed. Security key 2FA is not enabled for your account."
    SECURITY_KEY_PIN_NOT_SET_MESSAGE = "Two-factor authentication failed: Your security key has no PIN set"
    ...
    def __init__(
        ...
        safe_signal_connect(self._authenticate_button, "clicked", self._on_authenticate_button_clicked)
        ...
        safe_signal_connect(self._pin_code_entry, "changed", self._on_pin_code_entry_changed)
        safe_signal_connect(self._pin_code_entry, "activate", self._on_authenticate_button_clicked)
        ...
        self._pin_code_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        ...
        safe_signal_connect(
            self._pin_code_box_revealer, "notify::reveal-child", self._on_pin_code_box_revealer_notify_reveal_child
        )
    ...
    def _on_authenticate_button_clicked(self, _):
        ...
        if self._requesting_pin:
            ...
            logger.info(
                "Clicked on authenticate via security key", category="UI", subcategory="LOGIN-2FA", event="CLICK"
            )
            future = self._controller.generate_2fa_fido2_assertion(
                user_interaction=self, cancel_assertion=self._cancel_assertion
            )
            future.add_done_callback(lambda future: GLib.idle_add(self._on_2fa_fido2_assertion, future))
    ...
    def _on_2fa_fido2_assertion(self, future: Future):
        ...
        future.add_done_callback(lambda future: GLib.idle_add(self._on_2fa_fido2_submission_result, future))
    ...
    def _on_2fa_fido2_submission_result(self, future: Future):
        try:
            ...
            logger.warning(
                "Security key submission failed: %s",
                error,
                category="APP",
                subcategory="LOGIN-2FA",
                event="RESULT",
                exc_info=True,
            )
