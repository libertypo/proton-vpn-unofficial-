...
class LoginStack(Gtk.Stack):
    ...
    def __init__(self, controller: Controller, notifications: Notifications, overlay_widget: OverlayWidget):
        ...
        self.two_factor_auth_widget = TwoFactorAuthWidget(controller, notifications, overlay_widget)
        self.add_named(self.two_factor_auth_widget, "2fa_form")

        safe_signal_connect(self.login_form, "user-authenticated", self._on_user_authenticated)
        self.display_form(self.login_form)

        safe_signal_connect(
            self.two_factor_auth_widget, "two-factor-auth-successful", self._on_two_factor_auth_successful
        )

        safe_signal_connect(
            self.two_factor_auth_widget, "two-factor-auth-cancelled", self._on_two_factor_auth_cancelled
        )
