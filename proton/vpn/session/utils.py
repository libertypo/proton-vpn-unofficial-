...
class RefreshCalculator:
    ...
    def __init__(self, refresh_interval: int, refresh_randomness_in_percentage: float = None):
        """
    ...
    @staticmethod
    def get_expiration_time(refresh_interval: int, refresh_randomness: float = None, start_time: float = None) -> float:  # noqa: E501 pylint: disable=missing-function-docstring
...
async def rest_api_request(session, route, **api_request_kwargs):  # noqa: E501 pylint: disable=missing-function-docstring
    ...
    response = await session.async_api_request(route, **api_request_kwargs)
...
def semver_from_pep440(pep440_version: str) -> str:
    ...
    if ver.pre is not None:
        prerelease_mappings = {"a": "alpha", "b": "beta", "rc": "rc"}
