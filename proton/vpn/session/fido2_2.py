...
from fido2.client import AssertionSelection, DefaultClientDataCollector, Fido2Client
...
def create_client(device: CtapHidDevice, origin: str, user_interaction: Fido2UserInteraction) -> Fido2Client:
    ...
    return Fido2Client(  # pylint: disable=unexpected-keyword-arg
        device, collector, user_interaction=user_interaction
    )
...
def create_options(assertion_parameters: Fido2AssertionParameters) -> Options:
    ...
    user_verification = UserVerificationRequirement(assertion_parameters.user_verification)
    return Options(
