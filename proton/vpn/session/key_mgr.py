...
class KeyHandler:  # pylint: disable=missing-class-docstring
    ...
    def from_sk_file(cls, ed25519sk_file):  # pylint: disable=missing-function-docstring
        ...
        private_key = key.private_bytes(
            Encoding.Raw, PrivateFormat.Raw, encryption_algorithm=serialization.NoEncryption()
        )
    ...
    def __generate_key_pair(cls, private_key=None):
        if private_key:
            private_key = cryptography.hazmat.primitives.asymmetric.ed25519.Ed25519PrivateKey.from_private_bytes(
                private_key
            )
        else:
